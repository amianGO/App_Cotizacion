...
def migrate_from_excel(excel_path: str = None) -> None:
        ...
        try:
            ...
            for _, row in products_df.iterrows():
                ...
                if nombre:
                    try:
                        conn.execute(
                            f"INSERT OR IGNORE INTO {TABLE_PRODUCTS} (nombre, descripcion) VALUES (?, ?)",
                            (nombre, descripcion)
                        )
            ...
            for _, row in suppliers_df.iterrows():
                nombre = _normalize_text(row.get("Nombre", ""))
                correo = _normalize_text(row.get("Correo", ""))
                if nombre and correo and _is_valid_email(correo):
                    try:
                        conn.execute(
                            f"INSERT OR IGNORE INTO {TABLE_SUPPLIERS} (nombre, correo) VALUES (?, ?)",
                            (nombre, correo)
                        )
                    except Exception as e:
